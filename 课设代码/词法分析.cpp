...
static two_elements TOKEN[1000];//词法序列（二元式结构)
...
void printf(two_elements*token) {
	...
	float ck = 10;
	for (int i = 0; i < 1000; i++) {
		if (token[i].kind == def) break;//遇空则停止输出
		int j=0;
		float number=0;
		cout << '('<<i + 1<<')' ;
		if (i < 9)cout << ' ';
		...
		case C:cout << "常数             ";
			...
			break;
		case K:cout << "关键字           "; fvalue(keyword,token[i].value2); j = strlen(keyword[token[i].value2-1]); break;
		case P:cout << "界符             "; fvalue(delimiters,token[i].value2); j = strlen(delimiters[token[i].value2 - 1]); break;
		case Ch:cout << "字符             " << token[i].value1  ; j = strlen(token[i].value1); break;
		case St:cout << "字符串           " << token[i].value1; j = strlen(token[i].value1); break;
		}
		while (j < 14) {
			cout << " ";
			j++;
...
static void scanner() {//词法扫描
	...
	if ((ch >= 'a'&&ch <= 'z') || (ch >= 'A'&&ch <= 'Z')) {//进入关键字或变量判断
		...
		location1--;
		TOKEN[serial].kind = I;
		if (is_keyword(token)) {//查关键字表，否则填入变量表
			TOKEN[serial].kind = K;
